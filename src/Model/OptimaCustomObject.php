...
use \ObjectModel;
use \Db;
class OptimaCustomObject extends ObjectModel
{
    /**
    ...
    public function createDatabase()
    {
        ...
        foreach ($definition['fields'] as $field_name => $field)
        {
            ...
            if (isset($field['lang'])&&$multilang && $field['lang']) {
        ...
        Db::getInstance()->execute($sql);
         $sql='';
        $sql= 'ALTER TABLE ' . _DB_PREFIX_ . $definition['table'] . ' ADD UNIQUE (';
    ...
    if(!empty($unique))
     ...
     $sql='';
     $sql= 'CREATE INDEX  idx_'.$definition['table']. ' ON  ' . _DB_PREFIX_ . $definition['table'] . ' (';
